...
const ProductInformation = ({
  ...
  const renderStockBadge = (product) => {
    ...
    const currentStock = Math.max(0, (availableStock>1?availableStock-1:availableStock) - (cartQuantity - 1));
    console.log({currentStock,availableStock});
    
    const showWarning = showLowStockCount && minimumStockForWarning >= currentStock;
    return (
  ...
  console.log("ddd",state.modalData[0]?.stock);
        ...
        {state.modalData[0]?.store_name ? (
          router.pathname !== `/store/[id]` ? (
            <Stack direction="row" alignItems="center" spacing={0.5}>
              <Link
                href={getStoreRedirectURL(
                  state.modalData[0]?.store_details || {
